...
{
	/* Setting the number of player to 0. */
	game->maps.cnt.p = 0;
	game->maps.cnt.c = 0;
	game->maps.cnt.e = 0;
	game->maps.cnt.r = 0;
...
int	check_comp_isvalid(t_game *game)
{
/* Checking if the number of pieces is valid. */
	if (!(0 < game->maps.cnt.p && game->maps.cnt.p <= COMPO_P_MAX))
		return (FALSE);
	/* Checking if the number of collectibles is valid. */
	if (!(0 < game->maps.cnt.c && game->maps.cnt.c <= COMPO_C_MAX))
		return (FALSE);
	/* Checking if the number of enemies is valid. */
	if (!(0 < game->maps.cnt.e && game->maps.cnt.e <= COMPO_E_MAX))
		return (FALSE);
	/* Checking if the number of rows is valid. */
